...
func BenchmarkRequestN1(b *testing.B) {
	...
	sys := newTestSystem(1)
	s, _ := New(0, chainId, &sb.Config{N: 1, F: 0, BatchDurationNsec: 2000000000, BatchSizeBytes: 1, RequestTimeoutNsec: 20000000000}, sys.NewAdapter(0))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkRequestN4(b *testing.B) {
	...
	for i := uint64(0); i < N; i++ {
		a := sys.NewAdapter(i)
		s, err := New(i, chainId, &sb.Config{N: N, F: 1, BatchDurationNsec: 2000000000, BatchSizeBytes: 11, RequestTimeoutNsec: 20000000000}, a)
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkRequestN80(b *testing.B) {
	...
	for i := uint64(0); i < N; i++ {
		a := sys.NewAdapter(i)
		s, err := New(i, chainId, &sb.Config{N: N, F: (N - 1) / 3, BatchDurationNsec: 2000000000, BatchSizeBytes: 11, RequestTimeoutNsec: 20000000000}, a)
		if err != nil {
			b.Fatal(err)
		}
		repls = append(repls, s)
