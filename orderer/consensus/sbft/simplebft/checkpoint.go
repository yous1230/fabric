...
func (s *SBFT) handleCheckpoint(c *sb.Checkpoint, src uint64) {
	...
	if err != nil {
		s.logger.Infof("replica %d: checkpoint signature invalid for %d from %d", s.id, c.Seq, src)
		return
	}

	// TODO should we always accept checkpoints?
	if c.Seq != s.cur.subject.Seq.Seq {
		s.logger.Infof("replica %d: checkpoint does not match expected subject %v, got %v", s.id, &s.cur.subject, c)
		return
	}
	if _, ok := s.cur.checkpoint[src]; ok {
		s.logger.Infof("replica %d: duplicate checkpoint for %d from %d", s.id, c.Seq, src)
	}
	...
	if !reflect.DeepEqual(c.Digest, s.cur.subject.Digest) {
		s.logger.Warningf("replica %d: weak checkpoint %x does not match our state %x --- primary %d of view %d is probably Byzantine, sending view change",
	...
	s.logger.Infof("replica %d: request %s %s delivered on %d (completed common case)", s.id, s.cur.subject.Seq, hash2str(s.cur.subject.Digest), s.id)
