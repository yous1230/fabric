...
var _ = Describe("DiscoveryService", func() {
	...
	It("discovers network configuration information", func() {
		...
		for _, o := range network.Orderers {
			...
			mspConfig, err := msp.GetVerifyingMspConfig(network.OrdererOrgMSPDir(org), org.MSPID, "bccsp", nil)
			Expect(err).NotTo(HaveOccurred())
		...
		for _, p := range network.Peers {
			...
			mspConfig, err := msp.GetVerifyingMspConfig(network.PeerOrgMSPDir(org), org.MSPID, "bccsp", nil)
			Expect(err).NotTo(HaveOccurred())
			Expect(discoveredConfig.Msps[org.MSPID]).To(Equal(unmarshalFabricMSPConfig(mspConfig)))
		}
