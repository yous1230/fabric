...
func TestKeyGenInvalidInputs(t *testing.T) {
	...
	csp, err := NewWithParams("ECDSA", 256, "SHA2", &mocks.KeyStore{StoreKeyErr: errors.New("cannot store key")})
...
func TestKeyImportInvalidInputs(t *testing.T) {
	csp, err := NewWithParams("ECDSA", 256, "SHA2", &mocks.KeyStore{})
	assert.NoError(t, err)
...
func TestGetKeyInvalidInputs(t *testing.T) {
	...
	csp, err := NewWithParams("ECDSA", 256, "SHA2", &mocks.KeyStore{GetKeyErr: errors.New("cannot get key")})
	assert.NoError(t, err)
	...
	k := &mocks.MockKey{}
	csp, err = NewWithParams("ECDSA", 256, "SHA2", &mocks.KeyStore{GetKeyValue: k})
	assert.NoError(t, err)
