...
// baseDir/name
func NewCA(baseDir, org, name, country, province, locality, orgUnit, streetAddress, postalCode, cryptoConf string) (*CA, error) {
	var response error
	var ca *CA

	err := os.MkdirAll(baseDir, 0755)
	if err == nil {
		priv, signer, err := csp.GeneratePrivateKey(baseDir, cryptoConf)
		response = err
		if err == nil {
...
// generate a signed X509 certificate, default using ECDSA
func genCertificate(baseDir, name string, template, parent *x509.Certificate, pub,
	priv interface{}) (*x509.Certificate, error) {

	//create the x509 public cert
	certBytes, err := gcx.GetX509().CreateCertificate(rand.Reader, template, parent, pub, priv)
	if err != nil {
		return nil, err
	}
	...
	x509Cert, err := gcx.GetX509().ParseCertificate(certBytes)
	if err != nil {
		return nil, err
	}
	return x509Cert, nil
